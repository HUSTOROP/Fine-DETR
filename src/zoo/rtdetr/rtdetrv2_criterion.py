...
from .box_ops import box_cxcywh_to_xyxy, box_iou, generalized_box_iou, bbox_alpha_iou
...
class RTDETRCriterionv2(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        loss_giou = 1 - torch.diag(bbox_alpha_iou(\
            box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes), CIoU=True))
    ...
    # TODO
    def loss_background_l2norm(self, enc_topk_memory):
        alpha = 0.02
        losses = {}

        losses['loss_l2norm'] = alpha * torch.norm(enc_topk_memory[:,15:30,:], p=2, dim=2).mean() # [b, 15, 256]
        return losses


    def forward(self, outputs, targets, enc_topk_memory, **kwargs):
        ...
        # TODO
        losses.update(self.loss_background_l2norm(enc_topk_memory))
    ...
    def get_loss_meta_info(self, loss, outputs, targets, indices):
        ...
        if self.boxes_weight_format == 'iou':
            ...
            iou = torch.diag(bbox_alpha_iou(\
                box_cxcywh_to_xyxy(src_boxes.detach()), box_cxcywh_to_xyxy(target_boxes), CIoU=True))
        else:
