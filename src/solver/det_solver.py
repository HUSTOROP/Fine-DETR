...
class DetSolver(BaseSolver):
    ...
    def fit(self, ):
        ...
        for epoch in range(start_epcoch, args.epoches):
            ...
            if best_stat['coco_eval_bbox'] > best_map:
                print('save best state')
                if checkpoint_paths is not None and os.path.exists(checkpoint_paths[-1]):
                    os.remove(checkpoint_paths[-1])
                best_map = best_stat['coco_eval_bbox']
                if self.output_dir:
                    checkpoint_paths = [self.output_dir / 'last.pth']
                    # extra checkpoint before LR drop and every 100 epochs
                    if (epoch + 1) % args.checkpoint_freq == 0:
